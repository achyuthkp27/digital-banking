...
export default function CinematicAppFlow() {
  ...
  const tApp = useTranslations('AppFlow');
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: 'radial-gradient(circle at center, rgba(var(--accent-rgb), 0.05) 0%, rgba(3, 7, 18, 0) 70%)',
              ...
              {step === 1 && (
                ...
                >
                  <div style={{ width: '400px', padding: '48px', background: 'rgba(var(--color-invert-rgb), 0.02)', borderRadius: '24px', border: '1px solid rgba(var(--color-invert-rgb), 0.05)' }}>
                    ...
                    <h2 style={{ textAlign: 'center', fontFamily: 'var(--font-syne)', fontSize: '28px', color: 'var(--text-primary)', marginBottom: '32px' }}>{tApp('secureLogin')}</h2>
                ...
                >
                  ...
                  <div style={{ width: '280px', borderRight: '1px solid rgba(var(--color-invert-rgb), 0.05)', padding: '32px', background: 'rgba(var(--color-base-rgb), 0.2)', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {[tApp('sidebarOverview'), tApp('sidebarTransactions'), tApp('sidebarCards'), tApp('sidebarAnalytics'), tApp('sidebarSettings')].map((lbl, i) => (
                        <div key={i} style={{ height: '48px', borderRadius: '8px', padding: '0 16px', display: 'flex', alignItems: 'center', background: i === 0 ? 'rgba(var(--accent-rgb),0.1)' : 'transparent', border: i === 0 ? '1px solid rgba(var(--accent-rgb),0.2)' : 'none' }}>
                          <span style={{ color: i === 0 ? 'var(--accent)' : 'rgba(var(--color-invert-rgb), 0.5)', fontFamily: 'var(--font-syne)', fontSize: '15px', fontWeight: i === 0 ? 600 : 400 }}>{lbl}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ flex: 1, padding: '40px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                      <div>
                        <h1 style={{ fontFamily: 'var(--font-syne)', fontSize: '36px', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>{tApp('financialOverviewTitle')}</h1>
                        <p style={{ color: 'rgba(var(--color-invert-rgb), 0.5)', margin: '8px 0 0 0' }}>{tApp('financialOverviewSubtitle')}</p>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
                      {[
                        { l: tApp('totalBalance'), v: '$2,459,203', t: '+14.5%' },
                        { l: tApp('monthlySpend'), v: '$142,390', t: '-2.4%' },
                        { l: tApp('pendingYield'), v: '$12,450', t: '+1.2%' }
                      ].map((s, i) => (
                        <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + (i * 0.1) }} style={{ background: 'rgba(var(--color-invert-rgb), 0.02)', border: '1px solid rgba(var(--color-invert-rgb), 0.05)', borderRadius: '16px', padding: '24px' }}>
                          ...
                          <div style={{ color: s.t.startsWith('+') ? 'var(--accent)' : '#ef4444', fontSize: '13px' }}>{s.t} {tApp('thisMonth')}</div>
                        </motion.div>
                      ))}
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '24px', flex: 1 }}>
                      ...
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} style={{ flex: 1.5, background: 'rgba(var(--color-invert-rgb), 0.02)', border: '1px solid rgba(var(--color-invert-rgb), 0.05)', borderRadius: '20px', padding: '24px', display: 'flex', flexDirection: 'column' }}>
                         <div style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-syne)', fontSize: '18px', fontWeight: 600, marginBottom: '24px' }}>{tApp('cashFlow')}</div>
                      ...
                      >
                        <div style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-syne)', fontSize: '18px', fontWeight: 600, marginBottom: '24px' }}>{tApp('activeCards')}</div>
                            ...
                            return (
                              ...
                              >
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                  <div style={{ fontFamily: 'var(--font-syne)', fontWeight: 700, fontSize: '14px' }}>{tApp('corporate')}</div>
              ...
              {step === 5 && (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '48px' }}>
                     <div>
                        <h1 style={{ fontFamily: 'var(--font-syne)', fontSize: '36px', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>{tApp('wealthManagementTitle')}</h1>
                        <p style={{ color: 'rgba(var(--color-invert-rgb), 0.5)', margin: '8px 0 0 0' }}>{tApp('wealthManagementSubtitle')}</p>
                      </div>
                      <div style={{ background: 'rgba(var(--color-invert-rgb), 0.05)', padding: '12px 24px', borderRadius: '12px', border: '1px solid rgba(var(--color-invert-rgb), 0.1)' }}>
                        <div style={{ color: 'rgba(var(--color-invert-rgb), 0.5)', fontSize: '12px', marginBottom: '4px' }}>{tApp('totalNetWorth')}</div>
                  ...
                  <div style={{ display: 'flex', gap: '32px', flex: 1 }}>
                    ...
                    <div style={{ flex: 1, background: 'rgba(var(--color-invert-rgb), 0.02)', borderRadius: '24px', border: '1px solid rgba(var(--color-invert-rgb), 0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                       <svg width="240" height="240" viewBox="0 0 240 240">
                         ...
                         <motion.circle cx="120" cy="120" r="60" fill="none" stroke="var(--accent)" strokeWidth="20" strokeDasharray="377" initial={{ strokeDashoffset: 377 }} animate={{ strokeDashoffset: 200 }} transition={{ duration: 1.5, delay: 0.4, ease: 'easeOut' }} />
                       </svg>
                       <div style={{ position: 'absolute', textAlign: 'center' }}>
                         <div style={{ color: 'rgba(var(--color-invert-rgb), 0.5)', fontSize: '14px' }}>{tApp('assets')}</div>
                    ...
                    <div style={{ flex: 1.5, display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      {[
                        { name: tApp('equities'), val: '$8,204,100', color: '#3b82f6' },
                        { name: tApp('fixedIncome'), val: '$3,100,500', color: '#8b5cf6' },
                        { name: tApp('alternatives'), val: '$1,101,320', color: 'var(--accent)' }
                      ].map((item, i) => (
